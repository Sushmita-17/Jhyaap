  ...
  return (
    <div className={mapClass} style={{ height: mapHeight }}>
      <MapContainer center={VALLEY_CENTER} zoom={13} className="h-full w-full" scrollWheelZoom zoomControl>
        ...
        <div className="absolute left-1/2 transform -translate-x-1/2 bottom-16 z-[600] pointer-events-auto">
          <div className="w-[420px] max-w-[92vw] bg-white rounded-2xl shadow-xl p-2.5 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs font-semibold text-gray-900">{activeRider?.name || 'Raj Thapa'} <span className="text-amber-400 text-[10px]">★ 4.8</span></div>
                <div className="text-[10px] text-gray-500 mt-0.5">{activeRider?.phone || '9801234567'} · {activeRider?.vehicle || 'Ba 95 Pa 1234'}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="text-center">
                ...
                <div className="text-sm font-bold text-gray-900">{routeCoordinates?.length ? `${Math.round(routeCoordinates.reduce((acc, p, i, arr) => acc + (i>0? Math.hypot((p.lat - arr[i-1].lat)*111000, (p.lng - arr[i-1].lng)*100000):0),0))} m` : Math.round(activeRider?.distance||3200)} </div>
              </div>
              <div className="text-center">
                <div className="text-[10px] text-gray-500">ETA</div>
                <div className="text-sm font-bold text-gray-900">{routeCoordinates?.length ? `${Math.round((routeCoordinates.length>0? (routeCoordinates.length/100):12))} min` : `${Math.round(activeRider?.eta||12)} min`}</div>
              </div>
            </div>
            ...
            <div className="flex-1">
              <div className="flex items-center justify-end gap-1.5">
                ...
                <div className="flex items-center gap-1 text-[10px] text-gray-600">
                  ...
                  <div className="text-[9px] leading-tight">Order<br/><span className="font-semibold text-gray-800">{activeRider?.status === 'assigned' || activeRider?.status === 'picking' ? 'Pending' : 'Picked Up'}</span></div>
                </div>
                <div className="flex items-center gap-1 text-[10px] text-gray-600">
                  <span className="inline-flex items-center justify-center w-5 h-5 bg-emerald-50 rounded-full text-emerald-600 text-[8px]">🛵</span>
                  <div className="text-[9px] leading-tight">Status<br/><span className="font-semibold text-gray-800">In Transit</span></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="absolute bottom-3 right-3 z-[500] flex gap-2">
        ...
        <button type="button" onClick={() => mapInstance?.zoomIn()} className="rounded-xl bg-black/80 p-3 text-white shadow-lg hover:bg-black"><ZoomIn className="h-4 w-4" /></button>
        <button type="button" onClick={() => mapInstance?.zoomOut()} className="rounded-xl bg-black/80 p-3 text-white shadow-lg hover:bg-black"><ZoomOut className="h-4 w-4" /></button>
      </div>
    </div>
  );
}
