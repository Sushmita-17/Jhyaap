    ...
    <div className="mx-auto max-w-3xl space-y-6 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className={`panel space-y-6 p-6 backdrop-blur-xl border hover:border-[#C9A84C]/50 transition-all duration-500 hover:shadow-2xl hover:shadow-[#C9A84C]/20 relative overflow-hidden ${
        ...
        <div className="relative">
          ...
          />
          <label className={`relative mt-3 flex items-center gap-2 text-sm text-[#C9A84C] transition-colors ${
            isUploading ? 'opacity-65 cursor-wait' : 'cursor-pointer hover:underline hover:text-[#C9A84C]/80'
          }`}>
            <Upload className={`h-4 w-4 ${isUploading ? 'animate-spin' : ''}`} />
            {isUploading ? 'Uploading asset file...' : 'Upload image file'}
            <input
              type="file"
              accept="image/*"
              className="hidden"
