...
export default function OrderTrackingPage() {
  ...
  const statusTimeline = [
    { stage: 'placed', label: 'Order Placed', description: 'Your order has been received', icon: CheckCircle },
    { stage: 'confirmed', label: 'Confirmed', description: 'Order confirmed by store', icon: CheckCircle },
    { stage: 'preparing', label: 'Preparing', description: 'Your order is being prepared', icon: Clock },
    { stage: 'out_for_delivery', label: 'Rider Assigned', description: 'Rider assigned - waiting for pickup', icon: Truck },
    { stage: 'picked_up', label: 'Out for Delivery', description: 'Rider is on the way - live GPS active', icon: Truck },
  ...
  const isLive = order.status === 'preparing' || order.status === 'out_for_delivery';
          ...
          <div className="panel border-neon-amber/20 bg-neon-amber/5 p-5">
            ...
            {isLive && (
              <p className="mt-2 flex items-center gap-2 text-sm text-green-400">
                ...
                Live GPS auto-started - rider on the way
              </p>
            )}
            {!isLive && order.status !== 'delivered' && order.status !== 'cancelled' && (
              <p className="mt-2 text-sm text-night-400">
                {order.status === 'preparing'
                  ? 'Rider is picking your order - live map active at Jhyaap Station'
                  : 'Auto-updating... live map opens when rider is dispatched'}
              </p>
            )}
          </div>
