...
import { bearingBetween } from './deliveryLocations';
import { DELIVERY_RIDER_MARKER_ICON, RIDER_MARKER_SIZE } from './mapIcons';
...
export const RIDER_ICON_ANCHOR = [RIDER_MARKER_SIZE / 2, Math.round(RIDER_MARKER_SIZE * 0.81)];
...
export const createRiderIcon = createRiderLeafletIcon;
...
const stateClass = isDriving ? 'is-driving' : 'is-idle';
  ...
  // NO wheel spinning animation - both wheels must stay firmly grounded
  const wheelRolls = '';
...
export function createAnimatedRiderMarker(map, { position, heading = 0, showPulse, isDriving, isSelfView }) {
  const marker = L.marker([position.lat, position.lng], {
    icon: createRiderLeafletIcon({ heading, showPulse, isDriving, isSelfView }),
    zIndexOffset: 1000,
  }).addTo(map);
