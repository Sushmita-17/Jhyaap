...
export default function CategoryCarousel({
  ...
  return (
    <section className="relative rounded-lg border border-white/10 bg-[#16110F]/55 py-3 md:py-5 shadow-lg shadow-black/10">
      ...
      <div className="overflow-hidden px-3 sm:px-4 md:px-5" ref={emblaRef}>
        <div className="flex gap-2.5 sm:gap-3 md:gap-4 pb-1">
          {categories.map((category) => (
            <div
              key={category.id}
              onClick={() => onSelect(category.id)}
              className="group relative aspect-[5/4] overflow-hidden rounded-full cursor-pointer flex-[0_0_140px] sm:flex-[0_0_168px] md:flex-[0_0_190px]"
            >
              <img
                src={category.image}
                alt={category.name}
