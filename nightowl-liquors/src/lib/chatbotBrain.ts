...
function topProducts(products: Product[], category?: string, limit = 3) {
...
function latestActiveOrder(orders: Order[], userId?: string) {
...
export async function getChatbotReply(message: string, context: ChatContext): Promise<string> {
  ...
  if (matchAny(text, ['price', 'cost', 'how much'])) {
    ...
    if (product) {
      return `${product.name} is **Rs ${product.price.toLocaleString()}** (${product.volume}, ${product.abv}). ${product.inStock ? 'In stock ✅' : 'Out of stock'}`;
    }
  ...
  if (product) {
    return `**${product.name}**\nRs ${product.price.toLocaleString()} · ${product.volume} · ${product.abv}\nRating: ${product.rating}/5 (${product.reviews} reviews)\n${product.inStock ? '✅ In stock' : '❌ Out of stock'}`;
  }
