...
export const STORE_LOCATION: GeoPoint = {
  lat: 27.7074359,
  lng: 85.2853747,
...
export const VALLEY_MAP_CENTER: GeoPoint = {
  lat: 27.69,
  lng: 85.34,
...
export function estimateEtaMinutes(from: GeoPoint, to: GeoPoint, speedKmh = 22): number {
  const km = haversineKm(from, to);
  return Math.max(5, Math.round((km / speedKmh) * 60));
}

export function googleMapsDirectionsUrl(origin: GeoPoint, destination: GeoPoint): string {
  return `https://www.google.com/maps/dir/?api=1&origin=${origin.lat},${origin.lng}&destination=${destination.lat},${destination.lng}&travelmode=driving`;
}

export function googleMapsRiderTrackUrl(rider: GeoPoint, destination: GeoPoint): string {
