...
export function createRiderAtStore(): DeliveryRider {
...
export function riderLabel(rider: DeliveryRider): string {
  return `${rider.name} · ${rider.vehicle}`;
}

export function formatRiderSpeed(speed: number): string {
  return `${Math.round(speed)} km/h`;
}

export function statusLabel(status: DeliveryRider['status']): string {
  switch (status) {
    case 'picking':
...
export function trackingPhaseLabel(status: DeliveryRider['status'], orderStatus?: string): string {
