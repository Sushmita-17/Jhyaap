...
let ActualRiderMapOverlay: any = null;
let ActualAreaLabelOverlay: any = null;
...
function getRiderClass() {
  if (!ActualRiderMapOverlay) {
    ActualRiderMapOverlay = class extends google.maps.OverlayView {
      ...
      constructor(position: google.maps.LatLngLiteral, rider: DeliveryRider, isLive: boolean) {
        super();
      ...
      draw(): void {
        ...
        const marker = this.div.querySelector('.jhyaap-rider-marker') as HTMLElement | null;
        if (marker) {
      ...
      update(position: google.maps.LatLngLiteral, rider: DeliveryRider, isLive: boolean): void {
        this.position = position;
        this.rider = rider;
        this.isLive = isLive;
        if (this.div) {
...
function getAreaClass() {
  if (!ActualAreaLabelOverlay) {
    ActualAreaLabelOverlay = class extends google.maps.OverlayView {
      ...
      constructor(position: google.maps.LatLngLiteral, label: string, highlighted = false) {
        super();
        this.position = position;
        this.label = label;
...
export class RiderMapOverlay {
  ...
  update(position: google.maps.LatLngLiteral, rider: DeliveryRider, isLive: boolean) {
