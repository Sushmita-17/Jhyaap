...
import { Order, CartItem, Address, DeliveryRider } from '@/types';
...
interface OrdersState {
  ...
  updateOrderStatus: (orderId: string, status: Order['status'], userId?: string) => void;
  tickRiderPosition: (orderId: string) => void;
  tickActiveDeliveries: (onlyOrderId?: string) => void;
  advanceOrderPipeline: () => void;
  getOrders: () => Order[];
...
function applyRiderForDelivery(order: Order): Order {
...
function applyRiderPicking(order: Order): Order {
  if (!order.deliveryRider) return order;
...
function earnLoyaltyForOrder(order: Order, userId?: string) {
...
export const useOrdersStore = create<OrdersState>((set, get) => ({
  ...
  createOrder: (items, address, paymentMethod, discount, userId, pointsRedeemed = 0, notes) => {
    const subtotal = items.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
    const destination = getAreaCoordinates(address.area);
  ...
  updateOrderStatus: (orderId, status, userId) => {
    set((state) => {
      const updated = state.orders.map((order) => {
        ...
        let next: Order = { ...order, status, statusUpdatedAt: new Date().toISOString() };
  ...
  tickActiveDeliveries: (onlyOrderId?: string) => {
    set((state) => {
      const updated = state.orders.map((order) => {
        ...
        if (order.status === 'preparing' || order.status === 'confirmed') {
          ...
          return {
            ...
            deliveryRider: {
              ...
              status: (order.status === 'preparing' ? 'picking' : 'idle') as 'picking' | 'idle',
              speed: 0,
        ...
        const riderStatus: DeliveryRider['status'] = progress >= 1 ? 'arrived' : 'driving';
